...
public class TestServlet extends HttpServlet {
	...
	private static final String LOCATION = "D:\\Programok\\apache-tomcat-9.0.22\\webapps\\TestServlet\\WEB-INF\\config.conf";
   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      private boolean checkAuth(HttpServletRequest request){
		  String autValue = request.getHeader("Authorization");
		  if (autValue==null) {
			  return false;
		  } else {
			  if(!autValue.toUpperCase().startsWith("BASIC"))
				  return false;
			  else {
				  String userpass = new String(Base64.getDecoder().decode(autValue.substring(6))); //user pass
				  int counter = userpass.indexOf(":");
				  String observedUsername = userpass.substring(0,counter);
				  String observedPass = userpass.substring(counter+1);
				   if (observedUsername.equals(USERNAME) && observedPass.equals(PASSWORD)) 
					   return true;
			  }
		  }
		  return false;
